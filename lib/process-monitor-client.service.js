 ...
 */
async function submitProcess(tenantId, type, name = null, params, options = {}) {
  assert(tenantId);
  assert(type);
  ...
  if (options.scheduledDate) {
    if (!_.isDate(options.scheduledDate)) {
      throw new Error('Incorrect options.scheduledDate');
    }
    const processId = processService.formatSingleProcessId(tenantId, type, name) ;

    const lock = await queueLockService.getPendingQueueLock();
    const lock2 = await queueLockService.getScheduleQueueLock(processId);
    try {
      const existingProcess = await processService.findActiveProcessById(processId);
      if (existingProcess) {
        if (existingProcess.status === processService.PROCESS_STATUSES.PENDING || existingProcess.status === processService.PROCESS_STATUSES.SCHEDULED) {
          // if the process is pending or scheduled, it will be canceled, an new process will start below
          await processService.cancelProcess(existingProcess, 'New schedule submitted');
        } else {
          // if the process is in progress, it will be updated so that it runs the proper schedule the next run
          logger.info('%s: rescheduling existing process from %s to %s', existingProcess.display, moment(existingProcess.scheduledDate), moment(options.scheduledDate));
          existingProcess.scheduledDate = options.scheduledDate;
          existingProcess.intervalInSecs = options.intervalInSecs || 0;
          existingProcess.progressDescription = 'Rescheduled while in progress';
          return processService.updateOne(tenantId, existingProcess);
        }
      }
    } finally {
      lock.release();
    ...
    // const lock = await queueLockService.getScheduleQueueLock(processId);
    // try {
    //   const existingScheduledProcess = await processService.findScheduledProcessById(processId);
    //   if (existingScheduledProcess) {
    //     // Only one schedule process runs enterprise wide with this exact tenantId, type and name
    //     // to prevent spawning the same type of process and consume too many resources.
    //     logger.info('%s: rescheduling existing process from %s to %s', existingScheduledProcess.display, moment(existingScheduledProcess.scheduledDate), moment(options.scheduledDate));
    //     existingScheduledProcess.scheduledDate = options.scheduledDate;
    //     existingScheduledProcess.intervalInSecs = options.intervalInSecs || 0;
    //     // need to save
    //     return processService.updateOne(tenantId, existingScheduledProcess);
    //   }
    // } finally {
    //   lock.release();
    // }
  ...
  const process = new processService.TenantProcess({
    ...
    params,
    server: null,
    status: options.scheduledDate ? processService.PROCESS_STATUSES.SCHEDULED : processService.PROCESS_STATUSES.PENDING,
    scheduledDate: options.scheduledDate,
    intervalInSecs: options.intervalInSecs,
    single: options.single || false, // by default only multiple processes with this type and name can run in the cluster at a time
    progressDescription: options.scheduledDate ? `Scheduled start on ${moment(options.scheduledDate).format('MMM Do, hh:mm:ss A')}` : 'Pending'
  });
...
async function cancelProcesses(tenantId, type, name, reason = '') {
  if (!zerv.getRedisClient()) {
    throw new Error('REDIS_DISABLED');
  }
  const lock = await queueLockService.getScheduleQueueLock();
  const lock2 = await queueLockService.getPendingQueueLock();
  try {
    const processes = await processService.findByTenantIdAndTypeAndNameAndInPendingOrScheduled(tenantId, type, name);
    for (const process of processes) {
      await processService.cancelProcess(process, reason);
    }
    return processes;
  } finally {
    lock.release();
